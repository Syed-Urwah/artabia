...
const ImageFileEdit = ({ setImagefile, setArtwCoverImage,artw_cover_image ,images }) => {
    ...
    useEffect(() => {
        setArtwCover(`http://admin.artabiasa.com/storage/${artw_cover_image}`);
      }, [artw_cover_image]);

      useEffect(() => {
        if (images) {
            const imageUrls = images.map((img) => {
                const filename = img.img || '';
                return `http://admin.artabiasa.com/storage/${filename}`;
              });
              setImage(imageUrls);
              
              setImagefile([...images.map(img => img.filename)]);
        }

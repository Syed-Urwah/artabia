...
const EditArtwork = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    artw_atext: "",
    artw_description_ar: "",
    artw_description_en: ";sdjfl;",
    artw_price: 'sdfj',
    artw_length: "dklfj",
    artw_width: "fasdfjklsdj",
    artw_depth: '',
    artw_weight: '',
    artw_dimensions: "",
    artw_location: '',
  ...
  useEffect(() => {
    ...
    const getArtwork = async () => {
      setIsLoading(true)
      try {
        const { data } = await axios.post('http://admin.artabiasa.com/api/get-product-details', {
          pers_fk: getPayload() && getPayload().sub,
          artw_pk: product_id,
          api_password: process.env.REACT_APP_API_PASSWORD,
        })
        setIsLoading(false)
        console.log('getArtwork ->', data)
        if (data.status === 'true') setArtwork(data.data)
      } catch (error) {
        console.log(error)
        setIsLoading(false)
      }
  ...
  useEffect(() => {
    if (artwork) {
      ...
      setFormData({
        ...artwork,
        id: artwork.artw_pk,
        material_fk: artwork.material_fk.matr_pk,
        frame_fk: artwork.frame_fk.frame_pk,
        subcat_fk: artwork.subcat_fk.subcat_pk,
        cat_fk: artwork.cat_fk.cat_pk,
      ...
      {isLoading ?
        (
          ...
          <div className="lg:px-16 px-4 py-16">
            ...
            <div>
              <ImageFileEdit artw_cover_image={formData.artw_cover_image} images={formData.images} setImagefile={setImagefile} setArtwCoverImage={setArtwCoverImage} />
              {/* <h1 className="text-[#F21079] my-4">Artwork photos are required</h1> */}
              <CategoryEdit subcat_fk={formData.subcat_fk} cat_fk={formData.cat_fk} setCatFk={setCatFk} setSubCatFk={setSubCatFk} />
                ...
                <div className="grid gap-6 mb-6 md:grid-cols-12">
                  <div className="flex flex-row items-center">
                    <input
                      ...
                      type="radio"
                      value="cm"
                      checked={formData.artw_dimensions === 'cm' ? true : false}
                      onChange={(e) => {
                        handleInputChange(e);
                  ...
                  <div className="flex flex-row items-center mt-2">
                    <input
                      ...
                      type="radio"
                      value="in"
                      checked={formData.artw_dimensions === 'in' ? true : false}
                      onChange={(e) => {
                        handleInputChange(e);
                        resetFormErrors('artw_dimensions');
                      }}
                ...
                <div className="grid gap-6 mb-6 md:grid-cols-2">
                  <div className="mb-6">
                    ...
                    <textarea
                      ...
                      value={formData.artw_description_ar}
                      onChange={(e) => {
                        handleInputChange(e);
